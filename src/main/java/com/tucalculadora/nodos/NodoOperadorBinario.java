        ...
        public NodoExpresion simplificar() {
            ...
            if (left instanceof NodoConstante && right instanceof NodoConstante) {
                ...
                return switch (operador) {
                    ...
                    case '-' -> new NodoConstante(leftNum - rightNum);
                    case '*' -> new NodoConstante(leftNum * rightNum);
                    case '/' -> rightNum == 0 ? new NodoOperadorBinario('/', left, right) : new NodoConstante(leftNum / rightNum);
            ...
            if (operador == '*' && right instanceof NodoConstante && ((NodoConstante) right).number == 0) return new NodoConstante(0);
            if (operador == '*' && left instanceof NodoConstante && ((NodoConstante) left).number == 0) return new NodoConstante(0);
            if (operador == '/' && right instanceof NodoConstante && ((NodoConstante) right).number == 1) return left;
            if (operador == '/' && left instanceof NodoConstante && ((NodoConstante) left).number == 0) return new NodoConstante(0);
